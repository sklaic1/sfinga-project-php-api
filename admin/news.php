<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
		...
		$query  = "INSERT INTO news (title, description, picture, archive, public)";
		$query .= " VALUES ('" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', '" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', '', '" . $_POST['archive'] . "', '" . $_POST['public'] . "')";
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news') {
		$query  = "UPDATE news SET title='" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', description='" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', archive='" . $_POST['archive'] . "', public='" . $_POST['public'] . "'";
	...
	#Add news 
		...
		<form action="" id="news_form" name="news_form" method="POST" enctype="multipart/form-data">
			...
			<label for="archive">Public:</label>
	...
	#Edit news
		...
		<form action="" id="news_form_edit" name="news_form_edit" method="POST" enctype="multipart/form-data">
			...
			<label for="archive">Public:</label>
			<input type="radio" name="public" value="1"'; if($row['public'] == 1) { echo ' checked="checked"'; $checked_public = true; } echo ' /> YES &nbsp;&nbsp;
			<input type="radio" name="public" value="0"'; if($checked_public == false) { echo ' checked="checked"'; } echo ' /> NO
			...
			<table>
				...
				while($row = @mysqli_fetch_array($result)) {
					...
					<tr>
						...
						<td class="status">';
							if ($row['archive'] == 'Y') { print '<img src="img/inactive.png" alt="Archived" title="Archived" />'; }
                            else if ($row['archive'] == 'N') { print '<img src="img/active.png" alt="" title="" />'; }
							print ' ';
							if ($row['public'] == 0) { print '<img src="img/inactive.png" alt="Private" title="Private" />'; }
                            else if ($row['public'] == 1) { print '<img src="img/active.png" alt="Public" title="Public" />'; }
						print '
						</td>
					</tr>';
				}
